...
const generateCertificatePdf = async (data) => {
    ...
    const drawCentered = (page, font, text, size, y, color = textColor) => {
    ...
    drawCentered(page, bold, university_name, 28, y, titleColor); y -= 40;
    // Subtitle
    drawCentered(page, font, 'CERTIFICATE OF COMPLETION', 16, y, textColor); y -= 50;
    // Body
    drawCentered(page, font, 'This is to certify that', 14, y, textColor); y -= 30;
    // Student name prominent
    drawCentered(page, bold, student_name, 24, y, titleColor); y -= 30;
    // Lines describing degree + course
    drawCentered(page, font, 'has successfully completed the requirements for', 14, y, textColor); y -= 24;
    drawCentered(page, font, `${degree_type} in`, 14, y, textColor); y -= 24;
    drawCentered(page, bold, `${course_name}`, 14, y, textColor); y -= 40;

    // Details table-like rows (left aligned)
    const labelX = margin;
    ...
    for (const [label, value] of rows) {
        page.drawText(label, { x: labelX, y, size: 12, font: bold, color: textColor });
        page.drawText(value, { x: valueX, y, size: 12, font, color: textColor });
        y -= rowGap;
    }
    y -= 40;
    ...
    const rightLineX = margin + lineWidth + 80;
    const lineY = y;
    page.drawLine({ start: { x: leftLineX, y: lineY }, end: { x: leftLineX + lineWidth, y: lineY }, thickness: 1, color: textColor });
    page.drawLine({ start: { x: rightLineX, y: lineY }, end: { x: rightLineX + lineWidth, y: lineY }, thickness: 1, color: textColor });
    page.drawText('Authorized Signature', { x: leftLineX + 10, y: lineY - 14, size: 10, font, color: textColor });
    page.drawText('Date', { x: rightLineX + 10, y: lineY - 14, size: 10, font, color: textColor });
    page.drawText(university_name, { x: leftLineX + 10, y: lineY - 28, size: 10, font: bold, color: textColor });
    page.drawText(String(issue_date), { x: rightLineX + 10, y: lineY - 28, size: 10, font: bold, color: textColor });
    ...
    // QR area
    const qrSize = 108; // ~1.5 inch
    const qrX = margin;
    const qrY = 220; // lower area
    ...
    if (hashInQr) {
        ...
        const lines = [
            ...
            '',
            `Hash: ${hashInQr.slice(0, 20)}...`
        ];
        let ty = infoY;
        for (const [idx, line] of lines.entries()) {
            const f = idx === 0 ? bold2 : font2;
            const s = idx === 0 ? 12 : 10;
            page2.drawText(line, { x: infoX, y: ty, size: s, font: f, color: textColor });
            ty -= (idx === 0 ? 18 : 14);
        }
    }
...
// Serve dashboard as entry point
app.get('/', (req, res) => {
...
app.post('/verify', upload.single('pdf_file'), async (req, res) => {
    ...
    console.log('[VERIFY_UPLOAD] computed hash2', hash2, 'found rows', rows.length);
        ...
        else if (dbFound && blockchainRevoked) verificationTier = 'Record revoked on blockchain';
...
// (Removed duplicate ensureSchema, duplicate routes, and stray code)
